...
export class HrComponent implements OnInit {
  ...
  approveSelected(): void {
    ...
    const path = this.isTour ? 'tours/change-status' : this.isLeave ? `leaves/${[...this.selectedIds][0]}/approve` : 'attendances/approve';
    const payload = this.isTour ? { id: [...this.selectedIds], status: '1' } : this.isLeave ? {} : { id: [...this.selectedIds] };
  ...
  rejectSelected(): void {
    ...
    const path = this.isTour ? 'tours/change-status' : this.isLeave ? `leaves/${[...this.selectedIds][0]}/reject` : 'attendances/reject';
    const payload = this.isTour ? { id: [...this.selectedIds], status: '2' } : this.isLeave ? { remark_status: remark } : { id: [...this.selectedIds], remark_status: remark };
    this.hrService.post(path, payload).subscribe({
      next: message => {
        this.showToast(message, 'success');
        this.loadRows();
